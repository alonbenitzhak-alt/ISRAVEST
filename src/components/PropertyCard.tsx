...
const ADMIN_WHATSAPP = "972586836555";

function buildWhatsAppUrl(title: string, city: string, country: string) {
  const msg = encodeURIComponent(`שלום, אני מעוניין/ת בנכס: ${title} - ${city}, ${country}. ראיתי אותו באתר MANAIO ואשמח לקבל פרטים נוספים.`);
  ...
  return (
    <div className="bg-white rounded-3xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-400 border border-gray-100 group hover:-translate-y-1">
      ...
      <div className="p-5">
        ...
        {/* WhatsApp button */}
        <a
          href={buildWhatsAppUrl(displayTitle, property.city, property.country)}
          target="_blank"
